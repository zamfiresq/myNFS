...
// cross platform argument passing for rpc calls 
#ifdef __APPLE__
// macos 
#define ARG_STR(s)  (&(s))
#define ARG_REQ(r)  (&(r))
#define ARG_CH(c)   (&(c))
#else
// Linux expects char*, request, chunk
#define ARG_STR(s)  (s)
#define ARG_REQ(r)  (r)
#define ARG_CH(c)   (c)
#endif
...
char **safe_ls(CLIENT *clnt) {
    ...
    char **res = ls_1(ARG_STR(arg), clnt);
    if (!res) return NULL;
...
int safe_create(CLIENT *clnt, const char *filename) {
    ...
    char *argp = path;
    int *res = create_1(ARG_STR(argp), clnt);
    if (!res) {
...
int safe_delete(CLIENT *clnt, const char *filename) {
    ...
    char *argp = path;
    int *res = delete_1(ARG_STR(argp), clnt);
    if (!res) {
...
int safe_mkdir(CLIENT *clnt, const char *dirname) {
    ...
    char *argp = path;
    int *res = mynfs_mkdir_1(ARG_STR(argp), clnt);
...
int safe_remdir(CLIENT *clnt, const char *dirname) {
    ...
    char *argp = path;
    int *res = mynfs_remdir_1(ARG_STR(argp), clnt);
    if (!res) {
...
int safe_edit(CLIENT *clnt, const char *filename) {
    ...
    while (1) {
        chunk *res = mynfs_read_1(ARG_REQ(req), clnt);
        if (!res) {
            break;
        }
        if (res->data.data_len <= 0) {
    ...
    int *res = mynfs_write_1(ARG_CH(ch), clnt);
    free(ch.data.data_val);
    if (!res || *res != 0) {
...
int safe_chdir(CLIENT *clnt, const char *dirname) {
    ...
    // validare pe server
    char *argp = candidate;
    char **res = ls_1(ARG_STR(argp), clnt);
    if (!res) return -1;  
...
int main(int argc, char *argv[]) {
    ...
    server_addr.sin_port = 0; // port discovered via portmapper
    ...
    CLIENT *clnt = clntudp_create(&server_addr, NFS_PROGRAM, NFS_VERSION_1, timeout, &sock);
    if (clnt == NULL) {
        clnt_pcreateerror("clntudp_create failed");
        exit(1);
    }
#endif
